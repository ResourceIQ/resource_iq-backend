...
@router.get("/", response_model=[DashboardResponse],dependencies=[Depends(RoleChecker([Role.ADMIN,Role.MODERATOR]))])
def get_dashboard(session: SessionDep, _current_user: CurrentUser) -> DashboardResponse:
    """Get aggregated dashboard metrics."""
    return get_dashboard_data(session)


@router.get("/integrations/health", response_model=ConnectedIntegrationsCard,dependencies=[Depends(RoleChecker([Role.ADMIN, Role.MODERATOR]))])
...
@router.get("/github/prs/stats", response_model=GitHubPRStatsCard,dependencies=[Depends(RoleChecker([Role.ADMIN,Role.MODERATOR]))])
...
@router.get("/jira/tasks/stats", response_model=JiraTaskStatsCard,dependencies=[Depends(RoleChecker([Role.ADMIN, Role.MODERATOR]))])
def get_jira_stats(
    session: SessionDep, _current_user: CurrentUser
) -> JiraTaskStatsCard:
...
@router.get("/profiles/integrations", response_model=ProfileIntegrationsCard,dependencies=[Depends(RoleChecker([Role.ADMIN,Role.MODERATOR]))])
