...
async def get_closed_prs_context_per_author(session: SessionDep, author: GitHubUser) -> list[PullRequestContent]:
    try:
        github_manager = GithubIntegrationService(session)
...
async def get_closed_prs_context_all_authors(session: SessionDep) -> dict[str, list[PullRequestContent]]:
    try:
        github_manager = GithubIntegrationService(session)
