...
def create_team(session: SessionDep, team_in: TeamCreate):
    """Create a new team."""
...
def update_team(session: SessionDep, team_id: int, team_in: TeamUpdate):
    """Update a team."""
    service = TeamService(session)
...
def add_team_member(session: SessionDep, team_id: int, profile_id: int):
...
def remove_team_member(session: SessionDep, team_id: int, profile_id: int):
    """Remove a resource profile from a team."""
    service = TeamService(session)
    success = service.remove_member(team_id, profile_id)
    if not success:
        raise HTTPException(
            status_code=400, detail="Member not found in team"
        )
