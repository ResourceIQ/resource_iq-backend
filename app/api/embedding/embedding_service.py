...
class VectorEmbeddingService:
    def __init__(self, db: Session, use_api: bool = True):
        ...
        if use_api:
            ...
            logger.info(f"Using Jina API for embeddings with model: {self.embedding_model}")
        else:
            # Local embedding model
            try:
                from sentence_transformers import SentenceTransformer
                self.model = SentenceTransformer(settings.JINA_EMBEDDING_MODEL2)
                logger.info(f"Using local Jina model for embeddings: {settings.JINA_EMBEDDING_MODEL2}")
            except ImportError:
                raise ImportError("Install sentence-transformers for local embeddings: pip install sentence-transformers")
    ...
    def _generate_embeddings_api(self, texts: List[str]) -> List[List[float]]:
        ...
        try:
            ...
            logger.info(f"Calling Jina API with model: {self.embedding_model}, URL: {self.api_url}")
    ...
    def store_pr_contexts(
        self, 
        author: GitHubUser, 
        pr_contents: List[PullRequestContent]
    ) -> None:
        ...
        pr_contexts = [pr.context for pr in pr_contents]
        
        # Generate embeddings with fallback to local model if API fails
        try:
            embeddings = self.generate_embeddings(pr_contexts)
        except Exception as e:
            logger.warning(f"Batch embedding failed for {author.login}, processing individually: {str(e)}")
            embeddings = []
            for i, doc in enumerate(pr_contexts):
                try:
                    ...
                    logger.error(f"Failed to embed PR {pr_contents[i].number} for {author.login}: {str(doc_error)}")
                    logger.error(f"Problematic text preview: {doc[:200]}...")
                    # Skip this PR
                    continue
        
        # Store in database
        for pr, embedding in zip(pr_contents, embeddings):
            try:
                # Normalize embedding to 1536 dimensions if needed
                embedding = self._normalize_embedding_dimension(embedding)
                
                # Check if already exists
                existing = self.db.query(GitHubPRVector).filter(
                    GitHubPRVector.pr_id == str(pr.id)
                ).first()
                
                if existing:
                    # Update existing
                    existing.embedding = embedding
                    existing.context = pr.context
    ...
    def _normalize_embedding_dimension(self, embedding: List[float]) -> List[float]:
        ...
        if len(embedding) == target_dim:
            ...
            logger.warning(f"Embedding size {len(embedding)} exceeds {target_dim}, truncating")
    ...
    def store_all_authors_pr_contexts(
        ...
        total_stored = 0
        for author_login, pr_contents in authors_prs.items():
            if pr_contents:
    ...
    def search_similar_prs(
        ...
        try:
            ...
            if author_login:
                query_obj = query_obj.filter(GitHubPRVector.author_login == author_login)
